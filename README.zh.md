...
| `restart_delay_ms` | `1000` | 自动重启前的延迟：崩溃后固定；spawn 失败后按重试次数递增。 |
...
Bun 会自动从项目的 `cwd` 加载 `.env` 文件。不要把应用密钥放在 `bm2.toml` 里；请放在应用的环境或 `.env` 文件中。bm2 保留变量被显式注入每个实例的环境：配置解析时直接拒绝 `[env]` 中的保留名；Bun 自动加载 `.env` 时也不会覆盖进程环境中已存在的变量。
