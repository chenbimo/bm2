...
- `BM2_RUN_MODE` (always `"production"`: bm2 is a production-run tool, so managed apps can reliably detect they are under bm2)
...
The effective precedence is `BM2_APP_NAME` / `BM2_INSTANCE_ID` / `BM2_APP_INSTANCE` / `BM2_APP_PORT` / `BM2_RUN_MODE`, then `apps.env`, then Bun's project `.env`, then the inherited `PATH`, `HOME`, and `TMPDIR`. Environment names must use letters, digits, and underscores, cannot begin with a digit, and `BM2_APP_NAME`, `BM2_INSTANCE_ID`, `BM2_APP_INSTANCE`, `BM2_APP_PORT`, `BM2_RUN_MODE`, `PATH`, `HOME`, and `TMPDIR` are reserved. Values must be TOML strings. bm2 never writes environment values to state files, events, crash logs, or CLI output.
